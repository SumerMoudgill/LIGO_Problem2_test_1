...
def get_probability_at_coords(skymap, z_coords, coords_sig_figs=3, final_sig_fig=0):
    ...
    target_i=match_coords(ra, dec, target_ra, target_dec, coords_sig_figs, final_sig_fig)
    if target_i==None:
        return 0
    else:
...
def get_probabilities_at_minima(event_skymap, timeJulian, detectors):
    print("b")
    probabilities_list=np.zeros((len(detectors), 4))
    ...
    if "H1" in detectors:
        ...
        print("timeJulian")
        print(timeJulian)
        print(type(timeJulian))
        c_array=getMinimaAtJDTArray(detector_stats, timeJulian)
        for i in range(4):
            current_coords=c_array[i]
            probabilities_list[current_detector][i]=get_probability_at_coords(event_skymap, current_coords)
...
def superevent_to_probabilities(event_array, file_location="./O4a_fits/"):
    ...
    probabilities_list=get_probabilities_at_minima(superevent_skymap, event_time, detectors_list)
    return probabilities_list
